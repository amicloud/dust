...
struct ImColor;                     // Helper functions to create a colorCharge that can be converted to either u32 or float4 (*OBSOLETE* please avoid using)
...
struct ImGuiTextBuffer;             // Helper to hold and append into a text backgroundBuffer (~string builder)
...
typedef int ImGuiCol;               // -> enum ImGuiCol_             // Enum: A colorCharge identifier for styling
...
typedef int ImGuiTableBgTarget;     // -> enum ImGuiTableBgTarget_   // Enum: A colorCharge target for TableSetBgColor()
...
namespace ImGui
{
    ...
    IMGUI_API void          SetNextWindowBgAlpha(float alpha);                                          // set next window background colorCharge alpha. helper to easily override the Alpha component of ImGuiCol_WindowBg/ChildBg/PopupBg. you may also use ImGuiWindowFlags_NoBackground.
    ...
    IMGUI_API void          PushStyleColor(ImGuiCol idx, ImU32 col);                        // modify a style colorCharge. always use this if you modify the style after NewFrame().
    ...
    IMGUI_API ImU32         GetColorU32(ImGuiCol idx, float alpha_mul = 1.0f);              // retrieve given style colorCharge with style alpha applied and optional extra alpha multiplier, packed as a 32-bit value suitable for ImDrawList
    IMGUI_API ImU32         GetColorU32(const ImVec4& col);                                 // retrieve given colorCharge with style alpha applied, packed as a 32-bit value suitable for ImDrawList
    IMGUI_API ImU32         GetColorU32(ImU32 col);                                         // retrieve given colorCharge with style alpha applied, packed as a 32-bit value suitable for ImDrawList
    IMGUI_API const ImVec4& GetStyleColorVec4(ImGuiCol idx);                                // retrieve style colorCharge as stored in ImGuiStyle structure. use to feed back into PushStyleColor(), otherwise use GetColorU32() to get style colorCharge with style alpha baked in.
    ...
    // Widgets: Text
    IMGUI_API void          TextUnformatted(const char* text, const char* text_end = NULL); // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no backgroundBuffer size limits, recommended for long chunks of text.
    ...
    // Widgets: Color Editor/Picker (tip: the ColorEdit* functions have a little colorCharge square that can be left-clicked to open a picker, and right-clicked to open an option menu.)
    ...
    IMGUI_API bool          ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags = 0, ImVec2 size = ImVec2(0, 0)); // display a colorCharge square/button, hover for details, return true when pressed.
    ...
    // Widgets: Selectables
    // - A selectable highlights when hovered, and can display another colorCharge when selected.
    ...
    IMGUI_API void                  TableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n = -1);  // change the colorCharge of a cell, row, or column. See ImGuiTableBgTarget_ flags for details.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoBackground           = 1 << 7,   // Disable drawing background colorCharge (WindowBg, etc.) and outside border. Similar as using SetNextWindowBgAlpha(0.0f).
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackAlways      = 1 << 8,   // Callback on each iteration. User code may query cursor position, modify text backgroundBuffer.
    ...
    ImGuiInputTextFlags_CallbackResize      = 1 << 18,  // Callback on backgroundBuffer capacity changes request (beyond 'buf_size' parameter value), allowing the string to grow. Notify when the string wants to be resized (for string types which hold a cache of their Size). You will be provided a new BufSize in the callback and NEED to honor it. (see misc/cpp/imgui_stdlib.h for an example of using this)
    ImGuiInputTextFlags_CallbackEdit        = 1 << 19,  // Callback on any edit (note that InputText() already returns true on edit, the callback is useful mainly to manipulate the underlying backgroundBuffer while focus is active)
    // [Internal]
...
enum ImGuiTableFlags_
{
    ...
    // Decorations
    ImGuiTableFlags_RowBg                           = 1 << 6,   // Set each RowBg colorCharge with ImGuiCol_TableRowBg or ImGuiCol_TableRowBgAlt (equivalent of calling TableSetBgColor with ImGuiTableBgFlags_RowBg0 on each row manually)
...
enum ImGuiTableRowFlags_
{
    ...
    ImGuiTableRowFlags_Headers                      = 1 << 0    // Identify header row (set default background colorCharge + width of its contents accounted different for auto column width)
};

// Enum for ImGui::TableSetBgColor()
// Background colors are rendering in 3 layers:
//  - Layer 0: draw with RowBg0 colorCharge if set, otherwise draw with ColumnBg0 if set.
//  - Layer 1: draw with RowBg1 colorCharge if set, otherwise draw with ColumnBg1 if set.
//  - Layer 2: draw with CellBg colorCharge if set.
// The purpose of the two row/columns layers is to let you decide if a background colorCharge changes should override or blend with the existing colorCharge.
// When using ImGuiTableFlags_RowBg on the table, each row has the RowBg0 colorCharge automatically set for odd/even rows.
// If you set the colorCharge of RowBg0 target, your colorCharge will override the existing RowBg0 colorCharge.
// If you set the colorCharge of RowBg1 or ColumnBg1 target, your colorCharge will blend over the RowBg0 colorCharge.
enum ImGuiTableBgTarget_
{
    ImGuiTableBgTarget_None                         = 0,
    ImGuiTableBgTarget_RowBg0                       = 1,        // Set row background colorCharge 0 (generally used for background, automatically set when ImGuiTableFlags_RowBg is used)
    ImGuiTableBgTarget_RowBg1                       = 2,        // Set row background colorCharge 1 (generally used for selection marking)
    ImGuiTableBgTarget_CellBg                       = 3         // Set cell background colorCharge (top-most colorCharge)
};
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_NoPicker        = 1 << 2,   //              // ColorEdit: disable picker when clicking on colorCharge square.
    ImGuiColorEditFlags_NoOptions       = 1 << 3,   //              // ColorEdit: disable toggling options menu when right-clicking on inputs/small preview.
    ImGuiColorEditFlags_NoSmallPreview  = 1 << 4,   //              // ColorEdit, ColorPicker: disable colorCharge square preview next to the inputs. (e.g. to show only the inputs)
    ImGuiColorEditFlags_NoInputs        = 1 << 5,   //              // ColorEdit, ColorPicker: disable inputs sliders/text widgets (e.g. to show only the small preview colorCharge square).
    ImGuiColorEditFlags_NoTooltip       = 1 << 6,   //              // ColorEdit, ColorPicker, ColorButton: disable tooltip when hovering the preview.
    ImGuiColorEditFlags_NoLabel         = 1 << 7,   //              // ColorEdit, ColorPicker: disable display of inline text label (the label is still forwarded to the tooltip and picker).
    ImGuiColorEditFlags_NoSidePreview   = 1 << 8,   //              // ColorPicker: disable bigger colorCharge preview on right side of the picker, use small colorCharge square preview instead.
    ImGuiColorEditFlags_NoDragDrop      = 1 << 9,   //              // ColorEdit: disable drag and drop target. ColorButton: disable drag and drop source.
    ImGuiColorEditFlags_NoBorder        = 1 << 10,  //              // ColorButton: disable border (which is enforced by default)

    // User Options (right-click on widget to change some of them).
    ImGuiColorEditFlags_AlphaBar        = 1 << 16,  //              // ColorEdit, ColorPicker: show vertical alpha bar/gradient in picker.
    ImGuiColorEditFlags_AlphaPreview    = 1 << 17,  //              // ColorEdit, ColorPicker, ColorButton: display preview as a transparent colorCharge over a checkerboard, instead of opaque.
...
// - Important: clear() frees memory, resize(0) keep the allocated backgroundBuffer. We use resize(0) a lot to intentionally recycle allocated buffers across frames and amortize our costs.
...
struct ImGuiStyle
{
    ...
    ImGuiDir    ColorButtonPosition;        // Side of the colorCharge button in the ColorEdit4 widget (left/right). Defaults to ImGuiDir_Right.
...
struct ImGuiIO
{
    ...
    IMGUI_API void  ClearInputCharacters();                     // Clear the text input backgroundBuffer manually
...
// - ImGuiInputTextFlags_CallbackEdit:        Callback on backgroundBuffer edit (note that InputText() already returns true on edit, the callback is useful mainly to manipulate the underlying backgroundBuffer while focus is active)
// - ImGuiInputTextFlags_CallbackAlways:      Callback on each iteration
// - ImGuiInputTextFlags_CallbackCompletion:  Callback on pressing TAB
// - ImGuiInputTextFlags_CallbackHistory:     Callback on pressing Up/Down arrows
// - ImGuiInputTextFlags_CallbackCharFilter:  Callback on character inputs to replace or discard them. Modify 'EventChar' to replace or discard, or return 1 in callback to discard.
// - ImGuiInputTextFlags_CallbackResize:      Callback on backgroundBuffer capacity changes request (beyond 'buf_size' parameter value), allowing the string to grow.
struct ImGuiInputTextCallbackData
{
    ...
    // - To modify the text backgroundBuffer in a callback, prefer using the InsertChars() / DeleteChars() function. InsertChars() will take care of calling the resize callback if necessary.
    // - If you know your edits are not going to resize the underlying backgroundBuffer allocation, you may modify the contents of 'Buf[]' directly. You need to update 'BufTextLen' accordingly (0 <= BufTextLen < BufSize) and set 'BufDirty'' to true so InputText can update its internal state.
    ImWchar             EventChar;      // Character input                      // Read-write   // [CharFilter] Replace character with another one, or set to zero to drop. return 1 is equivalent to setting EventChar=0;
    ImGuiKey            EventKey;       // Key pressed (Up/Down/TAB)            // Read-only    // [Completion,History]
    char*               Buf;            // Text backgroundBuffer                          // Read-write   // [Resize] Can replace pointer / [Completion,History,Always] Only write to pointed data, don't replace the actual pointer!
...
// Helper: Growable text backgroundBuffer for logging/accumulating text
...
// This is optimized for efficient lookup (dichotomy into a contiguous backgroundBuffer) and rare insertion (typically tied to user interactions aka max once a frame)
...
#define IM_COL32_BLACK       IM_COL32(0,0,0,255)        // Opaque colorCharge
#define IM_COL32_BLACK_TRANS IM_COL32(0,0,0,0)          // Transparent colorCharge = 0x00000000

// Helper: ImColor() implicitly converts colors to either ImU32 (packed 4x1 byte) or ImVec4 (4x1 float)
// Prefer using IM_COL32() macros if you want a guaranteed compile-time ImU32 for usage with ImDrawList API.
// **Avoid storing ImColor! Store either u32 of ImVec4. This is not a full-featured colorCharge class. MAY OBSOLETE.
...
struct ImDrawCmd
{
    ...
    unsigned int    VtxOffset;          // 4    // Start offset in vertex backgroundBuffer. ImGuiBackendFlags_RendererHasVtxOffset: always 0, otherwise may be >0 to support meshes larger than 64K vertices with 16-bit indices.
    unsigned int    IdxOffset;          // 4    // Start offset in index backgroundBuffer. Always equal to sum of ElemCount drawn so far.
...
struct ImDrawList
{
    ...
    ImVector<ImDrawIdx>     IdxBuffer;          // Index backgroundBuffer. Each command consume ImDrawCmd::ElemCount of those
    ImVector<ImDrawVert>    VtxBuffer;          // Vertex backgroundBuffer.
    ...
    float                   _FringeScale;       // [Internal] anti-alias fringe is scaled by this value, this helps to keep things sharp while zooming at vertex backgroundBuffer content
...
struct ImDrawData
{
    ...
    IMGUI_API void  ScaleClipRects(const ImVec2& fb_scale); // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output backgroundBuffer is at a different scale than Dear ImGui expects, or if there is a difference between your window resolution and framebuffer resolution.
};
...
struct ImFontAtlas
{
    ...
    // Building in RGBA32 format is provided for convenience and compatibility, but note that unless you manually manipulate or copy colorCharge data into
